...
import { computeCategoryBreakdown, getPeriodRange, listYearsWithSpending } from "@/lib/reports";
import { DEFAULT_PERIOD_KEY } from "@/lib/reports/period";
import { ReportsScreen, type ReportGroup } from "@/components/reports/reports-screen";
...
async function ReportsContent({
  ...
  const [breakdown, years, groupsRes, accountsRes, categoriesRes, drillRes] = await Promise.all([
    computeCategoryBreakdown(supabase, {
      from: range.from,
      to: range.to,
      categoryIds,
    }),
    listYearsWithSpending(supabase),
    supabase
